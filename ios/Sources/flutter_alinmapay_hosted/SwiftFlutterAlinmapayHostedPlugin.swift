...
@objc public class SwiftFlutterAlinmapayHostedPlugin: NSObject, FlutterPlugin, PKPaymentAuthorizationViewControllerDelegate {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(name: "flutter_alinmapay_hosted", binaryMessenger: registrar.messenger())
        let instance = SwiftFlutterAlinmapayHostedPlugin()
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        self.authorizationCompletion = { [weak self] payment in
            self?.flutterResult?(payment)
        }
        self.authorizationViewControllerDidFinish = { [weak self] error in
        ...
        if PKPaymentAuthorizationViewController.canMakePayments(usingNetworks: paymentNetworks) {
            ...
            if let viewController = authorizationViewController {
                ...
                guard let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
                      ...
                      let viewControllerToPresent = rootViewController.getTopViewController() else {
                    return
                }
            ...
            self.authorizationViewControllerDidFinish?(error)
         }
    ...
    public func paymentAuthorizationViewController(_ controller: PKPaymentAuthorizationViewController, didAuthorizePayment payment: PKPayment, handler completion: @escaping (PKPaymentAuthorizationResult) -> Void) {
        ...
        self.authorizationCompletion?(PaymentData)
    }
    ...
    public func paymentAuthorizationViewControllerDidFinish(_ controller: PKPaymentAuthorizationViewController) {
        guard let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
              let window = windowScene.windows.first,
              let rootViewController = window.rootViewController,
              let viewControllerToDismiss = rootViewController.getTopViewController() else {
            return
        }
        viewControllerToDismiss.dismiss(animated: true, completion: {})
...
extension UIViewController {
    func getTopViewController() -> UIViewController? {
        ...
        if let navigationController = self as? UINavigationController {
            return navigationController.visibleViewController?.getTopViewController()
        }
        if let tabBarController = self as? UITabBarController {
            return tabBarController.selectedViewController?.getTopViewController()
        }
        return self
    }
}
